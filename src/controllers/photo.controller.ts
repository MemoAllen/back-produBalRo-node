...
import keysScripts from "../config/keys";
// Models
import Photo, { IPhoto } from "../models/Photo";
import User from "../models/User";
import PasswordResset from "../models/PasswordResset";
import CodeConfirm from "../models/Code-confirm";
...
export async function signup(req: Request, res: Response) {
  ...
  await transporter.sendMail({
    ...
    to: email,
    subject: "Conmfirm your account",
    html: `Click <a href="${url}">Here</a> to Confirm`,
  });
...
export async function signin(req: Request, res: Response) {
  ...
  const token = jwt.sign( {_id:user._id} , keysScripts.keys.secret,{expiresIn:'120s'});
  const codigo = Math.random().toString(20).substr(2, 12);
  const confirmCode = new CodeConfirm({
    email,
    codigo,
  });
...
await confirmCode.save();
  ...
  const url = `http://localhost:4200/confirm/`;
  await transporter.sendMail({
    from: "admin@gmail.com",
    to: email,
    subject: "Confirm Authentication",
    html: `Click <a href="${url}">Here</a> to write ${codigo}`,
  });
...
export async function confirmSign(  req: Request,
  res: Response) {

    const email = req.body.email;
  const codigo = Math.random().toString(20).substr(2, 12);
  
  const passwordReset = new CodeConfirm({
    email,
    codigo,
  });
